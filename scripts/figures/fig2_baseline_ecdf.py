...
coalition_heads = {"0:2": 0.406, "0:4": 0.520, "0:7": 0.329}
...
ax.fill_between(
    ...
    0,
    y_vals,
    where=(sorted_data <= baseline_mean + baseline_std)
    & (sorted_data >= baseline_mean - baseline_std),
    alpha=0.15,
    color="gray",
    label="Mean ± 1 SD",
)
...
for head, delta_ld in coalition_heads.items():
    ...
    ax.axvline(
        delta_ld, color=colors[head], linewidth=2.5, linestyle="--", alpha=0.9, zorder=5
    )
...
ax.text(
    ...
    bbox=dict(
        boxstyle="round,pad=0.2", facecolor="white", edgecolor="orange", linewidth=1.0
    ),
)
...
# Title
ax.set_title(
    "Statistical Validation: Coalition vs. Random Baseline",
    fontsize=11,
    fontweight="bold",
    pad=10,
)
