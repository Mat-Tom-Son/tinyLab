...
# Calculate ECE
ece_baseline = np.sum(
    bin_counts_baseline * np.abs(bin_centers - baseline_accuracy)
) / np.sum(bin_counts_baseline)
ece_ablated = np.sum(
    bin_counts_ablated * np.abs(bin_centers - ablated_accuracy)
) / np.sum(bin_counts_ablated)
...
ax.plot(
    bin_centers, baseline_accuracy, color="#E63946", linewidth=2, alpha=0.7, zorder=2
)
...
ax.scatter(
    ...
    edgecolors="darkblue",
    linewidths=1.5,
    label=f"Coalition Ablated (ECE = {ece_ablated:.3f})",
    marker="o",
    zorder=3,
)
ax.plot(
    bin_centers, ablated_accuracy, color="#457B9D", linewidth=2, alpha=0.7, zorder=2
)
...
ax.annotate(
    "",
    xy=(bin_centers[idx_annotate], ablated_accuracy[idx_annotate]),
    xytext=(bin_centers[idx_annotate], baseline_accuracy[idx_annotate]),
    arrowprops=dict(arrowstyle="<->", color="green", lw=1.5),
)
ax.text(
    bin_centers[idx_annotate] + 0.05,
...
# Title
ax.set_title(
    "Calibration Improvement from Coalition Ablation",
    fontsize=11,
    fontweight="bold",
    pad=10,
)
