...
Primary experiments run on macOS with Apple M‑series (MPS) backends. For GPT-2 Medium we additionally repeated the H1 suppressor battery on an NVIDIA CUDA backend (torch 2.9.0, CUDA 12.8) using the same corpora and seeds $\{0,1,2\}$; logit-difference means and layer-0 head rankings matched the MPS runs within numerical noise (see the CUDA validation bundle in the supplement). We release deterministic scripts, seeds, and data splits to enable exact replication.
...
Unless noted, we report means over $\geq$3 seeds with 95\% bootstrap CIs over prompts (5k resamples). For head ranking we estimate empirical $p$ under the layer-0 random-head null and control FDR via Benjamini–Hochberg. Compute: Apple M-series (MPS backend) for the primary runs, with a CUDA replication of the GPT-2 suppressor battery documented in the supplement.
...
\subsection{Variance-based suppressor probe (Pythia)}\label{sec:methods-vdi}
To test whether early-layer suppression has a principled geometric signature beyond GPT-2 and Mistral, we apply a small structural probe to Pythia models using a variance-based diagnostic.
For Pythia-2.8B and Pythia-410M we inject i.i.d.\ Gaussian noise into the token embeddings with standard deviation $\sigma$ proportional to the mean embedding norm, then measure how much the first few blocks amplify these perturbations.
Let $r_0$ and $r_1$ denote the residual stream immediately before and after a given block, and $r_0^{\mathrm{base}}, r_1^{\mathrm{base}}$ their values without noise.
We define noise-induced deviations $\delta_0 = r_0 - r_0^{\mathrm{base}}$ and $\delta_1 = r_1 - r_1^{\mathrm{base}}$ and estimate
\[
  \mathrm{Var}_\text{before} = \mathbb{E}\big[\lVert \delta_0\rVert^2\big], \qquad
  \mathrm{Var}_\text{after} = \mathbb{E}\big[\lVert \delta_1\rVert^2\big]
\]
over prompts, positions, and noise seeds.
The block-level variance dampening index is
\[
  \mathrm{VDI}_\text{full} \;=\; \frac{\mathrm{Var}_\text{before} - \mathrm{Var}_\text{after}}{\mathrm{Var}_\text{before}},
\]
so that negative values indicate net amplification.
To attribute variance shaping to individual heads we ablate each attention head $h$ in turn (zeroing its contribution at the block output), recompute $\mathrm{Var}_\text{after}^{(-h)}$, and define a per-head effect
\[
  \mathrm{vdi\_effect}[h] \;=\; \frac{\mathrm{Var}_\text{after}^{(-h)} - \mathrm{Var}_\text{after}}{\mathrm{Var}_\text{before}}.
\]
Positive values indicate heads whose removal increases variance (relative dampeners); large negative values indicate amplifiers.
In this paper we treat VDI as a structural metric only: it is not used to select GPT-2/Mistral suppressors, but to characterise early-layer variance shaping in Pythia and to assess how strongly such heads emerge at different depths and scales (Section~\ref{sec:findings}).
