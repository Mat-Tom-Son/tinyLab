...
\begin{table}[t]
    \centering
    \caption{Effect of layer-0 suppressor ablations on logit difference (LD). GPT-2 Medium: deterministic point estimates across three seeds (Apple M-series MPS). $^{\dagger}$Mistral-7B: single-seed estimates (seed~0; compute constraints). Positive $\Delta$LD indicates a stronger factual preference.}
    \label{tab:impact}
    \begin{tabular}{llcccc}
        ...
        \addlinespace
        Mistral 7B   & Facts          & $4.933^{\dagger}$ & $4.930^{\dagger}$ & $-0.003^{\dagger}$ & 0:22, 0:23 \\
        Mistral 7B   & Negation       & $0.384^{\dagger}$ & $0.609^{\dagger}$ & $+0.225^{\dagger}$ & 0:22, 0:23 \\
        Mistral 7B   & Counterfactual & $3.017^{\dagger}$ & $3.299^{\dagger}$ & $+0.282^{\dagger}$ & 0:22, 0:23 \\
        Mistral 7B$^{\ddagger}$   & Logic          & $0.335^{\dagger}$ & $0.293^{\dagger}$ & $-0.042^{\dagger}$ & 0:22, 0:23 \\
        \bottomrule
    \end{tabular}
    \vspace{0.5em}
...
\subsection{Foundational signals from cross-task sweeps}
