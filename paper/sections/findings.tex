...
\subsection{Early-layer variance shaping in Pythia}\label{sec:pythia-vdi}
The GPT-2/Mistral experiments establish that layer~0 suppressors are structural, high-impact, and form a hedging attractor.
To test whether early layers in other families show a comparable structural motif, we applied the variance-based diagnostic from Section~\ref{sec:methods-vdi} to Pythia models.

\paragraph{Layer-0 as a variance bottleneck.}
On Pythia-2.8B, the first block behaves as a strong noise amplifier: with small Gaussian perturbations to the embeddings (noise scale $\sigma$ chosen as a fraction of the mean embedding norm), the variance of the residual stream grows by two orders of magnitude across block~0 (block-level $\mathrm{VDI}_\text{full} \ll 0$).
Within that block, per-head VDI reveals a structured spectrum similar in spirit to the GPT-2 suppressors: a small set of heads act as relative dampeners (positive $\mathrm{vdi\_effect}$; removing them increases variance), and a small set act as amplifiers (large negative $\mathrm{vdi\_effect}$).
The ordering of heads along this dampener–amplifier axis is highly stable across noise scales ($\sigma\in\{0.03,0.05,0.08\}$), reinforcing the view of layer~0 as a variance bottleneck whose internal structure is carved out by training.

\paragraph{Depth and scale.}
Repeating the VDI probe at layers 4, 8, and 16 in Pythia-2.8B shows a sharp attenuation of the effect: block-level VDI moves toward zero, and per-head $\mathrm{vdi\_effect}$ values shrink, with no pronounced tails.
At these depths, heads still reshape variance slightly, but the strong expansion and variance shaping is concentrated at the earliest blocks.
A cross-scale check on Pythia-410M finds that its layer-0 block also amplifies noise, but the per-head VDI spectrum lacks robust positive tails: almost all heads have small negative or near-zero $\mathrm{vdi\_effect}$.
This suggests that strongly dampening heads are a scale-emergent motif: they appear clearly in Pythia-2.8B but only weakly, if at all, in smaller models.

\paragraph{Behavioural load-bearing in Pythia.}
To connect VDI to behaviour we ran small-sample drift experiments on Pythia-2.8B, comparing base runs to ablations of high-VDI and low-VDI heads.
Within-trajectory drift at the final layer does not show a simple monotone relationship to VDI: ablations of some high-VDI heads slightly reduce step-to-step drift rather than increase it.
However, paired base-versus-ablated metrics tell a clearer story: heads with larger positive $\mathrm{vdi\_effect}$ produce larger cosine divergence between base and ablated trajectories and larger KL divergence between the corresponding logit distributions.
In other words, early dampeners are \emph{load-bearing} for the downstream computation: removing them changes where the model goes in representation space, even if local drift metrics sometimes go down.
We treat these Pythia results as structural and mechanistic support for the bottleneck picture rather than as a full behavioural replication of the GPT-2/Mistral hallucination results.
...
\subsection{Mid- and late-layer motifs: binders and sharpeners}
Beyond layer~0 suppressors, two complementary motifs appear at depth.

\paragraph{Binder heads (mid-layers).} On GPT-2 Medium, single-head ablations over layers~8--12 on a binding probe (adjective--noun disambiguation) surface a consistent set of mid-layer heads whose removal lowers LD and accuracy; the five strongest are L10H14, L10H0, L8H0, L8H8, and L10H7 (synthetic dataset; $N{=}5{,}000$; top $\Delta$LDs range from $-0.30$ to $-0.10$). A natural-corpus variant (vehicles+furniture nouns) reproduces the ranking up to small swaps, confirming these are not synthetic artifacts.

\paragraph{Sharpeners (late layers).} Measuring last-position entropy per layer and scanning heads in the last three layers, we find candidates whose ablation \emph{raises} entropy (weakened commitment). On GPT-2 Medium, heads L23H6 and L23H4 produce the largest positive $\Delta H$ when ablated, while the baseline entropy profile shows a shallow dip in the high teens before rising toward the unembedding.

\paragraph{Causal rescue.} For both motifs, patching the head’s activations back into an ablated run (H6-style) restores LD and accuracy to within $\pm1$\% of baseline, providing a clean causal sanity check that the ablation effects are specific to the implicated heads rather than a global perturbation.

\paragraph{GPT-2 Small replicates the motifs.} On GPT-2 Small (12 layers) the synthetic binder sweep ($N{=}5{,}000$; float16 CUDA) flags \textbf{L11H0} as the strongest binder ($\Delta$LD$\approx-0.64$; $\Delta$acc$\approx-19$~pp), followed by \textbf{L7H1}, \textbf{L7H6}, \textbf{L6H9}, and \textbf{L10H0} ($\Delta$LD$\approx-0.29$ to $-0.23$). A corpus-derived variant ($N{=}5{,}000$) reproduces the ordering with smaller magnitudes. Causal-rescue on the top five heads restores both LD and accuracy to baseline within $\pm1$\% (2{,}048 examples), confirming head-specific effects. For late-layer sharpeners, the layer-entropy profile dips mid-network and rises near the unembedding; scanning the last three layers identifies \textbf{L11H0} and \textbf{L10H0} as top candidates on \emph{facts} (d\_entropy\_final $=+0.081$, $+0.074$). Per-probe rescues (facts/neg/cf/logic) reverse the ablation effect within $\pm1$\%. H6 path patching on Small shows the largest recoveries in the \emph{facts} window at layers 8--11 (per-layer $\Delta$LD $\approx 0.216$), with \emph{cf} peaking around layers 7--10 and earlier layers dominating \emph{neg}/\emph{logic}. The geometry battery on Small under L0 ablation (heads~\{0:2,0:4,0:7\}) shows small positive last-token entropy deltas across probes (facts $+0.03$, cf $+0.15$, neg $+0.17$, logic $+0.34$; non-extreme vs random controls), suggesting the strong output-flattening signature seen on Medium weakens at smaller scale. Together with GPT-2 Medium, this indicates the binding/sharpening division persists across scale, with indices shifting but roles conserved.
...
Mid- and late-layer motifs likewise replicate across GPT-2 scale: GPT-2 Small surfaces binders (L11H0, L7H1, L7H6, L6H9, L10H0) and late sharpeners (L11H0, L10H0) analogous to GPT-2 Medium, with causal rescues verifying specificity; see the consolidated summary.
...
Layer-0 suppressors are present at both GPT-2 Small (124M) and Medium (355M), as evidenced by strong H1 individual ablation effects. However, their functional role diverges qualitatively with model scale.

\paragraph{Small scale (\texorpdfstring{$d_\text{eff}\,\approx\,15$}{d_eff ≈ 15}).}
Individual ablations of heads~\{0:2, 0:4, 0:7\} on high-uncertainty tasks (negation, counterfactual, logic) reduce logit difference by \textbf{1.1–5.5\%} relative to baseline (per-head mean \(\Delta\)LD: \emph{neg} 0:2~\(\,-0.042\), 0:4~\(\,-0.101\), 0:7~\(\,-0.032\); \emph{cf} 0:2~\(\,-0.030\), 0:4~\(\,-0.039\), 0:7~\(\,-0.017\); \emph{logic} 0:2~\(\,-0.054\), 0:4~\(\,-0.067\), 0:7~\(\,-0.019\)).
The L0 triplet shows identical direction (mean \(\Delta\)LD, \emph{neg} \(\,-0.150\) [\(\,-0.202,\,-0.111\)], \emph{cf} \(\,-0.081\), \emph{logic} \(\,-0.097\) [\(\,-0.130,\,-0.057\)]), while \emph{facts} is weakly positive (\(\,+0.026\)).
H6 path mediation on \emph{facts} is minimal (\(\approx\,\)\textbf{5.9\%} [\(\,1.8\%,\,10.1\%\)] for layers~8–11).
Output-entropy deltas under L0 ablation are small and positive (\(\Delta H\,=\,+0.03\) to \(+0.34\)~nats).
Together this suggests that at Small, suppressors act as capacity-consuming gates rather than global attractor-setters; ablation can improve factuality on uncertainty-heavy tasks by forcing the model to reallocate limited capacity toward the task.

\paragraph{Medium scale (\texorpdfstring{$d_\text{eff}\,\approx\,50$--$80$}{d_eff ≈ 50–80}).}
Identical ablations improve logit difference by \textbf{40--85\%} across all probes (mean \(\Delta\)LD: \emph{facts} \(+0.40\), \emph{neg} \(+0.84\), \emph{cf} \(+0.85\), \emph{logic} \(+0.55\)).
H6 path mediation is robust (\(\approx\,\)\textbf{67\%} on \emph{facts}), and the geometry signature is strong (output-entropy \(\Delta H\,=\,-2.4\) to \(-3.8\)~nats).
Here suppressors stabilize a global hedging attractor; downstream layers inherit and reinforce the bias toward uncertainty.

\paragraph{Interpretation.}
Below a capacity threshold (\(d_\text{eff}\,\approx\,30\)--50?), suppressors are \emph{maladaptive}: they consume representational capacity that could otherwise solve the task.
At and above the threshold, suppressors become \emph{optimal}: they partition representation space into stable (hedged) and plastic zones, allowing the system to satisfy both predictive accuracy and calibration.
This is consistent with our constrained predictive geometry framing: constraint-induced geometry depends critically on effective capacity \(d_\text{eff}\). Suppressors emerge at all scales, but their role—from emergency brake to global basin—is capacity-contingent.

\begin{table}[t]
  \centering
  \caption{Cross-scale summary: early-layer coalition and downstream dynamics. Small numbers are GPT-2 Small (124M); Medium are GPT-2 Medium (355M).}
  \label{tab:capacity-contingent}
  \begin{tabular}{lccc}
    \toprule
    Metric & Small (124M) & Medium (355M) & Interpretation \\
    \midrule
    L0 triplet \(\Delta\)LD (neg)   & \(\,-0.150\) [\(\,-0.202,\,-0.111\)] & \(+0.842\) & Sign flip: maladaptive \(\to\) adaptive \\
    L0 triplet \(\Delta\)LD (logic) & \(\,-0.097\) [\(\,-0.130,\,-0.057\)] & \(+0.552\) & Sign flip: maladaptive \(\to\) adaptive \\
    H6 mediation (facts)              & \(\,5.9\%\) [\(\,1.8\%,\,10.1\%\)] & \(\,67\%\,\pm\,4\%\) & Attractor basin only at scale \\
    Output entropy \(\Delta H\)     & \(+0.03\) to \(+0.34\) & \(-2.4\) to \(-3.8\) & Geometry scales with capacity \\
    Interpretation                    & Local gates; capacity-limited & Global attractor; stable basin & Phase-transition regime \\
    \bottomrule
  \end{tabular}
\end{table}
