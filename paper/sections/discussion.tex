...
We studied GPT-2 Medium and Mistral-7B in depth, and added a small structural probe on Pythia models.
In Pythia-2.8B the first block forms a strong variance bottleneck with a clear spectrum of variance-dampening and variance-amplifying heads, while deeper blocks and a smaller Pythia-410M variant show much weaker per-head structure.
This suggests that early-layer variance shaping is concentrated at the first bottleneck and becomes more pronounced at larger scales.
Mapping suppressors and related motifs across GPT-3, Llama, Qwen, and other families---and testing whether variance-based diagnostics predict suppressor behaviour there---is necessary before claiming full universality.
...
The Pythia variance probe suggests that strong early-layer dampeners are scale-emergent and concentrated at the first bottleneck; a natural next step is to study their formation in small transformers trained from scratch and test whether perturbing them can causally shift the timing of circuit phase transitions (e.g., induction heads or circular structures).
...
This bottleneck-constrained, conserved-function pattern is consistent with thermodynamic views of hierarchy in neural systems, where functional structure is tied to nonequilibrium dynamics and time-asymmetry.
In the Thermodynamics of Mind framework, for example, Kringelbach et al.~\cite{kringelbach2024thermodynamics} quantify hierarchy in brain states via time-irreversibility of whole-brain activity.
Our contribution makes that story concrete at a different scale and substrate: we identify a specific early-layer circuit implementing a factuality–hedging trade-off in transformers and show that bottleneck geometry forces it into layer~0, where it forms an operational attractor that can be steered.
...
Primary experiments use deterministic Apple M-series (MPS) kernels; while we observed identical seeds across runs, hardware differences can introduce small numerical drift. To check for gross backend artefacts we repeated the GPT-2 Medium suppressor battery on an NVIDIA CUDA backend and recovered the same logit-difference band and layer-0 head rankings (see the CUDA validation bundle in the supplement), though Mistral-7B remains evaluated only on MPS due to VRAM constraints. We rely on byte-pair token cleanup when constructing the hedge/booster lexicon, so residual tokenization artifacts may remain. We augmented single-token probes with span-aware, teacher-forced sequence metrics (Section~\ref{sec:findings}) and observed the same early-layer bias with partial downstream recovery across all four tasks, supporting robustness beyond single-token evaluation. Open questions remain for fully free-running generation and longer contexts.
