...
\begin{itemize}
    ...
    \item \textbf{Hardware.} Apple M-series (M3 Max) with macOS for all primary experiments; GPT-2 suppressor batteries additionally validated on a single NVIDIA GPU (torch 2.9.0, CUDA 12.8) with matching metrics and head rankings. All runs use deterministic settings where available (no dropout, fixed seeds).
