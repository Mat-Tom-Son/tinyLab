...
HOOKS = [
    (f"blocks.{layer}.attn.hook_z", hook_builder(layer))
...
def per_prompt_delta_ld(model: HookedTransformer, rows: List[dict]) -> np.ndarray:
    ...
    base = (
        base_logits[torch.arange(base_logits.size(0)), torch.tensor(target_ids)]
        - base_logits[torch.arange(base_logits.size(0)), torch.tensor(foil_ids)]
    )
    abl = (
        abl_logits[torch.arange(abl_logits.size(0)), torch.tensor(target_ids)]
...
def bootstrap_ci(
    x: np.ndarray, n_boot: int = 5000, seed: int = 0
