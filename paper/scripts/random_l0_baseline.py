...
def main() -> None:
    ...
    baseline_single_l0 = singles[
        (~singles["head"].isin(suppressor_heads)) & (singles["layer"] == 0)
    ]["delta"].to_numpy()
    baseline_single_l1 = singles[singles["layer"] == 1]["delta"].to_numpy()
    suppressor_means = (
        singles[singles["head"].isin(suppressor_heads)]
        .groupby("head")["delta"]
        .mean()
        .to_dict()
    )
    ...
    single_percentiles = {
        head: float((baseline_single_l0 < val).mean())
        for head, val in suppressor_means.items()
    }
    pair_percentiles = {
        pair: float((sim_pairs < val).mean()) for pair, val in pair_means.items()
    }
    ...
    for head, val in suppressor_means.items():
        display_val = val
        if head == "2":
            facts_val = singles[(singles["head"] == "2") & (singles["tag"] == "facts")][
                "delta"
            ]
