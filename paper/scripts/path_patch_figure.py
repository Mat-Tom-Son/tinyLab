...
head2 = next(
    entry for entry in partial_data if entry["name"] == "facts_partial_patch_head2"
)
...
nodes = {"suppressor": (0.08, 0.5), "target": (0.48, 0.5), "logit": (0.88, 0.5)}

box = dict(
    boxstyle="round,pad=0.35", facecolor="white", edgecolor="black", linewidth=2.0
)
...
ax.annotate(
    "",
    xy=(nodes["target"][0], nodes["target"][1] + 0.08),
...
ax.text(
    nodes["suppressor"][0],
    0.73,
    f"Full ablation: ΔLD = {fmt_signed(full_delta)}",
    ha="center",
    va="center",
    fontsize=10,
)
...
ax.text(
    0.75,
    0.62,
    f"Reverse patch at L11: ΔLD = {fmt_signed(reverse_delta)}",
    ha="center",
    va="center",
    fontsize=10,
)
