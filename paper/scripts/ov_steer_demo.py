...
def ece_from_logits(
    target_ids: np.ndarray, foil_ids: np.ndarray, logits: torch.Tensor, n_bins: int = 10
) -> float:
...
def run_eval(
    model: HookedTransformer, rows: List[dict], alpha: float, vec: torch.Tensor
) -> Tuple[float, float]:
    ...
    with torch.no_grad():
        if alpha == 0.0:
            ...
            logits = model.run_with_hooks(
                toks, fwd_hooks=[("blocks.0.hook_resid_post", steer_hook)]
            )[:, -1, :]
    targ = logits[torch.arange(logits.size(0)), torch.from_numpy(target_ids)]
    foil = logits[torch.arange(logits.size(0)), torch.from_numpy(foil_ids)]
