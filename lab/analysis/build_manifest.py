...
def collect(prefix: str) -> dict:
    ...
    out = {
        "summary_table": (
            str(r / f"{prefix}_summary_table.csv")
            if (r / f"{prefix}_summary_table.csv").exists()
            else None
        ),
        "trio_percentiles": (
            str(r / f"{prefix}_trio_percentiles.json")
            if (r / f"{prefix}_trio_percentiles.json").exists()
            else None
        ),
        "head_rankings": {},
        "ov_reports": [],
