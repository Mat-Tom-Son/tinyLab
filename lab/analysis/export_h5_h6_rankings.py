...
def export_h5_triplets(outdir: Path) -> Path | None:
    ...
    for p in paths:
        ...
        if not {"heads", "label", "metric", "value", "layer", "run_id"}.issubset(
            df.columns
        ):
            continue
        ...
        if df.empty:
            continue
        g = (
            df.groupby(["run_id", "layer", "heads", "label"])["value"]
            .mean()
            .reset_index()
        )
...
def export_h6_layers(outdir: Path) -> Path | None:
    ...
    for p in paths:
        df = pd.read_parquet(p)
        if not {"granularity", "metric", "value", "layer", "run_id"}.issubset(
            df.columns
        ):
            continue
        df = df[df["metric"] == "logit_diff"]
